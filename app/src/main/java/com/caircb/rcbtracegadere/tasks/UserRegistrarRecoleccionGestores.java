...
import com.caircb.rcbtracegadere.models.request.RequestManifiestoPadreGestores;
...
public class UserRegistrarRecoleccionGestores extends MyRetrofitApi implements RetrofitCallbacks {
    ...
    public UserRegistrarRecoleccionGestores(Context context,
        ...
        if (model != null) {
         ...
         /*   if (estadotransportista == 1) {
           ...
           /* rq.setUrlFirmaResponsableEntrega(firmaTecnicoGenerador != null ? (path + "/" + firmaTecnicoGenerador.getUrl()) : "");
            ...
            rq.setGestores(gestoresRequest());
        }
        return rq;
    }
    ...
    private List<RequestManifiestoPadreGestores> gestoresRequest(){
        List<RequestManifiestoPadreGestores> resp = new ArrayList<>();
        List<LotePadreEntity> det = MyApp.getDBO().lotePadreDao().manifiestoPadreHijos(idAppManifiesto);
        if(det.size()>0){
            for (LotePadreEntity d:det){
                resp.add(new RequestManifiestoPadreGestores(d.getIdManifiestosHijo(),
                                                        d.getIdManifiestoDetalleHijo(),
                                                        d.getIdManifiestoDetallePadre(),
                                                        d.getIdDesecho(),
                                                        d.getPeso(),
                                                        d.getBultos()));
            }
